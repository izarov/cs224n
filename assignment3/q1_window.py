...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        for i in range(len(sentence)):
            left_window = [start]*abs(min(0, i-window_size)) + sentence[i-window_size:i]
            right_window = sentence[i+1:i+window_size+1] + [end]*max(0, (i+window_size+1-len(sentence)))
            features = left_window + sentence[i:i+1] + right_window
            flattened = [x for sublist in features for x in sublist]
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embeddings_tensor = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(embeddings_tensor, self.input_placeholder)
