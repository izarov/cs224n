...
class GRUCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_z = tf.get_variable('W_z', (self._state_size, self._state_size),
                                  tf.float32, initializer)
            W_r = tf.get_variable('W_r', (self._state_size, self._state_size),
                                  tf.float32, initializer)
            W_o = tf.get_variable('W_o', (self._state_size, self._state_size),
                                  tf.float32, initializer)
            U_z = tf.get_variable('U_z', (self.input_size, self._state_size),
                                  tf.float32, initializer)
            U_r = tf.get_variable('U_r', (self.input_size, self._state_size),
                                  tf.float32, initializer)
            U_o = tf.get_variable('U_o', (self.input_size, self._state_size),
                                  tf.float32, initializer)
            b_z = tf.get_variable('b_z', (self._state_size,),
                                  tf.float32, tf.constant_initializer(0))
            b_r = tf.get_variable('b_r', (self._state_size,),
                                  tf.float32, tf.constant_initializer(0))
            b_o = tf.get_variable('b_o', (self._state_size,),
                                  tf.float32, tf.constant_initializer(0))
