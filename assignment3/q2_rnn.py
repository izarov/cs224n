...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        pad_length = max(0, max_length-len(sentence))
        sentence_length = min(max_length, len(sentence))
        ret.append((sentence[:sentence_length] + [zero_vector]*pad_length,
                    labels[:sentence_length]+[zero_label]*pad_length,
                    [True]*sentence_length+[False]*pad_length))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        ce = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=preds, labels=self.labels_placeholder)
